...
def _preprocess_yolo(img, input_shape):
    ...
    # Returns
        preprocessed img: float32 numpy array of shape (3, H, nms_thresholdW)
    """
...
class PostprocessYOLO(object):
    ...
    def process(self, outputs, resolution_raw, conf_th):
        ...
        # print(" (OUTPUTS) ", outputs_reshaped)
        ...
        if len(boxes_xywh) > 0:
            ...
            boxes[:, [0, 2]] = np.clip(boxes[:, [0, 2]], 0.0, float(img_w - 1))
            boxes[:, [1, 3]] = np.clip(boxes[:, [1, 3]], 0.0, float(img_h - 1))
    ...
    def _nms_boxes(self, boxes, box_confidences):
        ...
        # print("SIZE Ordered:", len(ordered))
        keep = list()
