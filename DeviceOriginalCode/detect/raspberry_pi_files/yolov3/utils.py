...
from raspberry_pi_files.yolov3.configs import *
from raspberry_pi_files.yolov3.yolov3 import *
...
def detect_image(Yolo, numpyImage, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    clss = []
    nbbox = []
    # get score and class from bbox
    for i, bbox in enumerate(bboxes):

        confs.append(bbox[4])
        clss.append(int(bbox[5]))
        nbbox.append([bbox[0], bbox[1], bbox[2], bbox[3]])
        
    return nbbox, confs, clss
